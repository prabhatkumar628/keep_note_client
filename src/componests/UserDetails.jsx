...
export const UserDetails = ({ setUserDetails }) => {
  ...
  useEffect(() => {
    ...
    if (user?.avatar?.original) {
      setPicPreview(
        `${import.meta.env.VITE_API_BASE_URL}${user.avatar.original}`
      );
    } else {
    ...
    <>
      ...
      >
        ...
        <div className="flex items-center justify-between px-5 py-4 border-b">
          <h3 className="font-semibold text-lg text-gray-800">User Profile</h3>
          <button
            onClick={() => setUserDetails(false)}
        ...
        <div className="px-6 py-8 text-center overflow-y-auto h-[calc(100%-64px)] sm:h-auto">
          ...
          {errorData && (
            <div className="text-red-400">
              {errorData?.response?.data?.message ?? "ok"}
            </div>
          )}
          {newPic && (
            <button
          ...
          {/* User Info */}
          <h4 className="mt-5 text-xl font-semibold text-gray-800">
          ...
          <div className="mt-8 flex flex-col gap-3">
            ...
            <button
              ...
              className="w-full py-3 rounded-xl border border-gray-300 text-gray-700 font-medium hover:bg-gray-100 transition"
            >
              Logout
            </button>
          </div>
      ...
      {editProfile && (
        <div className="min-h-dvh w-full fixed top-0 left-0 z-50 flex items-center justify-center bg-gray-100 px-4">
          {/* Card */}
          <div className="w-full max-w-sm aspect-square bg-white rounded-2xl shadow-xl p-6 flex flex-col justify-center relative">
            <button
              onClick={() => {
                setEditProfile(false);
                setUserDetails(false);
              }}
              className="w-9 h-9 rounded-full hover:bg-gray-100 grid place-items-center absolute right-2 top-2"
            >
              <RxCross2 className="text-xl text-gray-700" />
            </button>
            ...
            <div className="mb-3">
              ...
              <input
                ...
                className="w-full px-4 py-2.5 rounded-xl border border-gray-300 focus:ring-2 focus:ring-gray-900 focus:border-gray-900 outline-none"
              />
            </div>
